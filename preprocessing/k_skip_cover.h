...
4 - it is covered 
(ie EVERY u-v path should have length (edges) >= k-1 AND should path through at least 1 vertex in V*)
*/
...
struct Comp_sp_pq {
    bool operator()(const pair<pair<int, int>, pair<double, int> >& pair1, const pair<pair<int, int>, pair<double, int> >& pair2) const {
        return pair1.second.first > pair2.second.first; //ascending order by their distance to the root
    }
};
...
class SP_Tree {
    public:
        ...
        priority_queue<pair<pair<int,int>, pair<double, int> > , vector<pair<pair<int, int>, pair<double, int> > >, Comp_sp_pq> sp_pq; //this will store the edges sorted in ascending order to their "distance" from the rootfor the tree construction
        SP_Tree(int root_id): root(root_id) {
            ...
            sp_pq.push(make_pair(make_pair(-1, root), make_pair(0.0, 0)));
        }
...
/* runs Dijkstra's shortest path algorithm on a given subset of the graph and for every SP computed that has k-1 edges, checks if the path passes through a cober node or not
