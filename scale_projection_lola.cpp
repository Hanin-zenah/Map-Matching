...
// double Euc_distance::euc_dist(double lat1, double lon1, double lat2, double lon2, 
                                // double x_scale, double y_scale, double lon_min, double lat_min) {
                                    // 
    // double x1 = degree_to_radian(lon1 - lon_min)*x_scale;
    // double x2 = degree_to_radian(lon2 - lon_min)*x_scale;
// 
    // double y1 = lat_mercator_proj(lat1 - lat_min)*y_scale;
    // double y2 = lat_mercator_proj(lat2 - lat_min)*y_scale;
// 
...
void Euc_distance::calc_edge_cost(Graph* graph, double x_scale, double y_scale) {
    ...
    for(int i = 0; i < graph -> n_edges; i++) {
        ...
        // src.lat= lat_mercator_proj(src.lat,graph -> min_lat,x_scale);
        // trgt.lat=lat_mercator_proj(trgt.lat,graph -> min_lat,x_scale);
        // src.longitude= lat_mercator_proj(src.longitude,graph -> min_long,y_scale);
        // trgt.longitude=lat_mercator_proj(trgt.longitude,graph -> min_long,y_scale);
